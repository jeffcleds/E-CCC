...
@php
    ...
    foreach ($enrollment->enrollmentSchedules as $schedule) {
        $totalUnits += $schedule->schedule->subject->units;
    }
@endphp
...
<body class="w-full min-h-screen h-full flex flex-col items-center ">
    ...
    <div class="w-full max-w-full h-full flex flex-col items-center justify-center p-4 text-[12px]">
           ...
           <div class="w-full flex flex-col font-serif">
               ...
               <table class="mt-4 border -mx-4 text-sm">
                   ...
                   <tbody>
                        @foreach ($enrollment->enrollmentSchedules as $schedule)
                            <tr class="border">
                                <td class="border px-1 py-0.5">{{ $schedule->schedule->subject->code }}</td>
                                <td class="border px-1 py-0.5">{{ $schedule->schedule->subject->description }}</td>
                                <td class="border px-1 py-0.5 text-center">{{ $schedule->average }}</td>
                                <td class="border px-1 py-0.5 text-center">{{ $schedule->schedule->subject->units }}</td>
                            </tr>
                        @endforeach
                        <tr class="border">
        ...
        <div class="w-full h-full flex flex-col items-center mt-12 text-left px-16 text-base">
            <div class="w-full flex flex-col font-serif">
                <p class="mt-4">&emsp; &ensp;&ensp;&ensp;&ensp;&ensp;&ensp;&ensp; This certification is issued this {{ today()->isoFormat('Do') }} day of {{ today()->format('M') }}, {{ today()->format('Y') }} at Calabanga Community College,
